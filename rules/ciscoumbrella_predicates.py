...
def is_domain_safe(action_input: Dict, ctx) -> bool:
    return llm_judge("Verify if the domain to be unblocked is safe", action_input, ctx)

def is_critical_domain(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the domain to be blocked is critical for business operations", action_input, ctx)

def is_policy_in_use(action_input: Dict, ctx) -> bool:
    return llm_judge("Verify if the policy to be deleted is currently in active use", action_input, ctx)

def has_policy_conflict(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the new policy conflicts with existing policies", action_input, ctx)
