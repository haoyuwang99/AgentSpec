...
def has_important_task(action_input: Dict, ctx) -> bool:
    return llm_judge("check if the task to be deleted has important information", action_input, ctx)

def is_critical_update(action_input: Dict, ctx) -> bool:
    return llm_judge("check if the update removes or alters critical information", action_input, ctx)
