...
def is_website_trusted(action_input: Dict, ctx) -> bool:
    return llm_judge("Determine if the website is trusted for auto-filling personal information.", action_input, ctx)

def is_contact_verified(action_input: Dict, ctx) -> bool:
    return llm_judge("Verify if the contact email is trusted for sharing passwords.", action_input, ctx)

def is_export_location_secure(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the export location is secure and not publicly accessible.", action_input, ctx)
