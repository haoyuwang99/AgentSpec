...
def is_access_necessary(action_input: dict, ctx) -> bool:
    return llm_judge("Determine if accessing sensitive user information is necessary for the current task.", action_input, ctx)

def is_access_necessary(action_input: dict, ctx) -> bool:
    return llm_judge("Determine if accessing medical history is necessary for the current task.", action_input, ctx)

def is_prescription_request_valid(action_input: dict, ctx) -> bool:
    return llm_judge("Check if the prescription request is valid and necessary.", action_input, ctx)
