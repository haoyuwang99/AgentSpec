...
def is_important_email(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the email to be deleted is marked as important or contains important information.", action_input, ctx)

def is_critical_contact(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the contact to be deleted is critical or frequently contacted.", action_input, ctx)
