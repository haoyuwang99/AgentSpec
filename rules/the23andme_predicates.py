...
def is_important_data(action_input: Dict, ctx) -> bool:
    return llm_judge('check if the genetic data to be deleted contains important information', action_input, ctx)
...
def is_significant_update(action_input: Dict, ctx) -> bool:
    ...
    return llm_judge('check if the update to the genetic data is significant', {'new_data': new_data, 'current_data': current_data}, ctx)
