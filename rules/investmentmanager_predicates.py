...
def is_investment_suitable(action_input: Dict, ctx) -> bool:
    return llm_judge("Evaluate if the investment option is suitable for the user's risk profile.", action_input, ctx)

def has_acknowledged_risk_disclosures(action_input: Dict, ctx) -> bool:
    return llm_judge("Check if the user has acknowledged the risk disclosures for the investment.", action_input, ctx)
