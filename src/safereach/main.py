...
def build_model(logs: List[List[Any]], abs:Abstraction):
    states = sorted(abs.state_space)
    state_idx = {s: i for i, s in enumerate(states)}
    state_interpret = {s: abs.decode(s) for s in states}
    K = len(states)
...
LOG_DIR = 'embodied/samples/'
MODEL_DIR = 'embodied/dtmcs/'
