...
class BasePromptExecutor:
    ...
    def _run(self, inputs: Dict[str, Any], **kwargs) -> Any:
        ...
        if self._stop_at == "prompt":
            return prompt
        output = self._llm.generate_prompt(prompt)
...
class BasePromptExecutorWithCritique(BasePromptExecutor):
    ...
    def _run(self, inputs: Dict[str, Any], **kwargs) -> List[Any]:
        ...
        if self._stop_at == "prompt":
            return prompt   
        base_messages = [ChatPromptValue(messages=p) for p in prompt]
        
        raw_output = self._llm.generate_prompt(base_messages)
        if self._stop_at == "llm":
            return raw_output
        ...
        for i in range(self._critique_rounds):
            try:
                ...
                raw_output = self._critique_llm.generate_prompt(critique_prompt)
                results.append(
